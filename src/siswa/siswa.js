...
function Status(props){
  switch (props.value) {
    ...
    case 1:
      return <span className="badge badge-success-light border border-success f-8 bolder py-1 px-3 badge-pill">Aktif</span>;
    case 2:
      return <span className="badge badge-info-light border border-info f-8 bolder py-1 px-2 badge-pill">Lulus</span>;
    case 3:
      return <span className="badge badge-light border border-gray f-8 bolder py-1 px-2 badge-pill">Daftar</span>;
    case 4:
      return <span className="badge badge-warning-light border border-warning f-8 bolder py-1 px-2 badge-pill">Pindahan</span>;
    default:
...
class Siswa extends React.Component {
  ...
  componentDidMount() {
    ...
    this._isMounted && axios.get(con.api+'/siswa', {headers:con.headers, params:{page:this.state.siswa_page}})
    .then(res => {
  ...
  scroll(e){
    if (e.target.scrollHeight - e.target.parentElement.offsetHeight === e.target.scrollTop) {
      axios.get(con.api+'/siswa', {headers:con.headers, params:{page:this.state.siswa_page + 1, q:this.state.siswa_search}})
      .then(res => {
  ...
  searcSiswa(e){
    axios.get(con.api+'/siswa', {headers:con.headers, params:{q:e.target.value}})
    .then(res => {
            ...
            <div className="card-header bg-white b-0 px-3 py-2">
              <div className="row align-items-center">
                ...
                <div className="col-auto pr-0 text-right">
                  <Link to="/siswa/add" className="alert alert-info py-1 px-2 pointer f-9 bolder"><i className="la la-user mr-1"></i>Tambah Siswa</Link>
                </div>
              </div>
            </div>
            <div className="card-body p-0">
                        ...
                        this.state.siswa.map(
                          (r, index) => (
                            <tr key={index}>
                              ...
                              <td className="f-8 bolder text-nowrap"><Status value={r.status} /></td>
