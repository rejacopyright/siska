...
class Siswa extends React.Component {
  ...
  componentDidMount() {
    ...
    this._isMounted && axios.get(con.api+'/siswa/daftar', {headers:con.headers, params:{page:this.state.siswa_page}})
    .then(res => {
  ...
  scroll(e){
    if (e.target.scrollHeight - e.target.parentElement.offsetHeight === e.target.scrollTop) {
      axios.get(con.api+'/siswa/daftar', {headers:con.headers, params:{page:this.state.siswa_page + 1, q:this.state.siswa_search}})
      .then(res => {
  ...
  searcSiswa(e){
    axios.get(con.api+'/siswa/daftar', {headers:con.headers, params:{q:e.target.value}})
    .then(res => {
